...
def run_analysis_single(n_clicks, raw_payload, analysis_types,
    ...
    if show_vol or show_dd:
        fig_ind.update_yaxes(
            ...
            secondary_y=True,
            side="right",
            title_standoff=15  # Add space to prevent overlap with axis label
        )
    ...
    if row3_needed:
        ...
        fig_ind.add_trace(go.Bar(
            x=time, y=feats["macd_hist"], 
            name="MACD Hist",
            marker=dict(opacity=0.7, line=dict(width=0)),
            width=86400000  # 1 day in milliseconds for proper bar width
        ), row=cur_row, col=1, secondary_y=False)
        fig_ind.add_trace(go.Scatter(
            x=time, y=feats["macd"], 
            mode="lines", 
    ...
    # Count number of traces to estimate legend height
    num_traces = len(fig_ind.data)
    # Estimate legend height: ~25px per row, with 2-3 rows for many indicators
    legend_rows = max(2, (num_traces // 7) + 1)  # ~7 items per row
    legend_height = legend_rows * 30 + 20  # 30px per row + padding
    ...
    fig_ind.update_layout(
        ...
        font=dict(color="rgba(255,255,255,0.9)"),
        margin=dict(
            t=150 + legend_height,  # Large top margin for legend and title above
            r=60,  # Right margin for secondary y-axis label (prevent overlap)
        ...
        legend=dict(
            orientation="h",
            yanchor="bottom",
            y=1.15,  # Position well above the chart area (outside plotting area)
            xanchor="center",
            x=0.5,
            itemwidth=25,  # Smaller items to fit more
        ...
        title=dict(
            ...
            xanchor="center",
            font=dict(size=16),
            y=1.08,  # Position between legend and chart
            yanchor="top"
        ),
        xaxis=dict(gridcolor="rgba(255,255,255,0.1)"),
        yaxis=dict(gridcolor="rgba(255,255,255,0.1)")
    ...
    indicators_container = html.Div([
        ...
        dcc.Graph(figure=fig_ind, config={"displayModeBar": False}, style={"height":"700px"})
    ], style={
        "background":"rgba(255,255,255,0.05)", "borderRadius":"16px",
