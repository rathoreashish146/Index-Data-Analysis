...
app.layout = html.Div(
    [
        ...
        dcc.Location(id="url"),
        html.Div(id="page-content"),
        dcc.Interval(id="progress-interval", interval=100, disabled=True, n_intervals=0)
    ],
    id="app-container",
    style={"fontFamily":"system-ui, -apple-system, Segoe UI, Roboto, Helvetica, Arial, sans-serif",
           "minHeight":"100vh","padding":"0", "margin":"0",
...
# =============================
# Loading Overlay Callbacks
# =============================

# Show loading overlay when analyze buttons are clicked
app.clientside_callback(
    """
    function(n_clicks_analyze, n_clicks_x_analyze) {
...
app.clientside_callback(
    """
    function(results_style, x_results_style) {
        const triggered = window.dash_clientside.callback_context.triggered;
        if (triggered.length === 0) {
            return window.dash_clientside.no_update;
        }
        
        // Check if results container became visible
        if ((results_style && results_style.display !== 'none') || 
            (x_results_style && x_results_style.display !== 'none')) {
    ...
    Input("x-results-container", "style"),
    prevent_initial_call=True
)
...
# Enable progress interval when loading
app.clientside_callback(
    """
    ...
    Input("loading-overlay", "className"),
    prevent_initial_call=True
)
